...
void RMSerialDriver::sendData(const auto_aim_interfaces::msg::Target::SharedPtr msg)
{
  ...
  try {
    ...
    geometry_msgs::msg::Point p_w_center;
    if (a_n != 3) {
    double armor_yaw[4];
    double r = 0;
    ...
    // double time_delay = dis / bullet_speed + 0.05;
      double time_delay;
      double CENTER_YAW = center_yaw;
      double xa = xc - r * cos(CENTER_YAW);
      double ya = yc - r * sin(CENTER_YAW);

      Eigen::Vector3d xyz = {xa, za, ya};
      double pitch_offset_center = dynamicCalcPitchOffset(xyz);
      std_msgs::msg::Float64 time_stemp;
      time_stemp.data = (this->now() - msg->header.stamp).seconds();
      time_delay = sqrt(xa*xa+ya*ya+za*za)/shoot_speed + time_fix_ + time_stemp.data;
    int idx = 0;
    ...
    //   tar_position[i].y = yc - r * sin(tmp_yaw);
      ...
      //////////////计算定点击打需要瞄准的装甲板///////////////////////////////////////////////////////////////
      //装甲板的长边0.135m
      double a = type == "small" ? small_length : large_length;
      double b = type == "small" ? small_z : large_z;
      double x1 = xa + a * sin(CENTER_YAW);
      double y1 = ya - a * cos(CENTER_YAW);
      double x2 = xa - a * sin(CENTER_YAW);
      double y2 = ya + a * cos(CENTER_YAW);
      double z1 = za + b;
      double z2 = za - b;
      //double a_distance = (sqrt(x1 * x1 + y1 * y1 + za * za) + (x2 * x2 + y2 * y2 + za * za)) / 2;
      double yaw1 = atan2(y1, x1) / M_PI * 180 + yaw_fix_;
      double yaw2 = atan2(y2, x2) / M_PI * 180 + yaw_fix_;
      double pitch1 = -atan2(z1, sqrt(xa * xa + ya * ya)) / M_PI * 180 - pitch_offset_center + pitch_fix_;
      double pitch2 = -atan2(z2, sqrt(xa * xa + ya * ya)) / M_PI * 180 - pitch_offset_center + pitch_fix_;
      if (yaw1 > yaw2) {
        double t1 = yaw1;
        yaw1 = yaw2;
        yaw2 = t1;
      ...
      //double a_distance = p_w.x * p_w.x + p_w.y * p_w.y + p_w.z * p_w.z;

      //////////////////////////////定点击打逻辑/////////////////////////////
       packet.distance = sqrt(xa * xa + ya * ya + p_w.z * p_w.z);
      if ((fabs(msg->v_yaw) > V_THRES ) && (packet.distance> 0 )) {
        std::cout << "1:::::::::::::::::::::::::::::::::::" << std::endl;
        packet.yaw_angle = atan2(ya, xa) / M_PI * 180 + yaw_fix_;
        if (packet.yaw_angle > 360)
          packet.yaw_angle -= 360;
        else if (packet.yaw_angle < 0)
          packet.yaw_angle += 360;
        packet.pitch_angle =
          -atan2(p_w.z, sqrt(xa * xa + ya * ya)) / M_PI * 180 + 90 - pitch_offset_center + pitch_fix_;
      }
      
      //////////////////////////////跟踪击打逻辑/////////////////////////////
      else {
        packet.yaw_angle = atan2(p_w.y, p_w.x) / M_PI * 180 + yaw_fix_;
        //packet.yaw_angle = atan2(p_w.y, p_w.x) / M_PI * 180 + 0.2;
        if (packet.yaw_angle > 360) {
          packet.yaw_angle -= 360;
        } else if (packet.yaw_angle < 0) {
          packet.yaw_angle += 360;
        }
        packet.pitch_angle = -atan2(p_w.z, sqrt(p_w.x * p_w.x + p_w.y * p_w.y)) / M_PI * 180 + 90 -
                             pitch_offset_center + pitch_fix_;
      ...
      //pre_yaw_error预测角度与电机角度差值
      double pre_yaw = atan2(p_w.y, p_w.x) / M_PI * 180 + yaw_fix_;
      //double pre_yaw_error = fabs(pre_yaw - receive_yaw_);
      //pitch_error瞄准角度与电机角度差值
      double pitch_error = abs(receive_pitch_ - packet.pitch_angle + 90);
      //yaw_error瞄准角度与电机角度差值
      ...
      if (packet.detect_flag != 0) {
        if (
          yaw_error < (atan((a/2) / packet.distance)) / M_PI * 180 &&
          pitch_error < (atan((b/2) / packet.distance)) / M_PI * 180) {
          if (fabs(msg->v_yaw) > V_THRES) {
            if (
              (pre_yaw >= yaw1 && pre_yaw <= yaw2) &&
              (receive_pitch_ >= pitch1 && receive_pitch_ <= pitch2))
              packet.shoot_flag = 1;
          } else
            packet.shoot_flag = 1;
        }
      ...
      //std::cout<<"yawfix:    "<<yaw_fix_<<std::endl;
      //std::cout<<"pitchfix:    "<<pitch_fix_<<std::endl;
      //std::cout<<"flag:    "<<packet.shoot_flag<<std::endl;
  }
  else {
     double armor_yaw[4];
      double r = 0;
      ...
      ////////////////////////////////////////////////////////
      //double time_delay;
      double OUTPOST_YAW = center_yaw;
      double xa = xc - r * cos(OUTPOST_YAW);
      double ya = yc - r * sin(OUTPOST_YAW);
      p_w_center.x = xa;
      p_w_center.y = ya;
      Eigen::Vector3d xyz2 = {xa, za, ya};
      double result = dynamicCalcPitchOffset(xyz2);
      std::cout<<"pitch dyni::" << result<<std::endl;
       std::cout<<"receive:yaw    "<<receive_pitch_<<std::endl;
      std_msgs::msg::Float64 time_stemp;
      time_stemp.data = (this->now() - msg->header.stamp).seconds();
      double time_delay_outpost =  (sqrt(xa * xa + ya * ya + za * za) / shoot_speed) + outpost_time_fix_ + time_stemp.data;
      std::cout << "time_delay_outpost:::::::::::::::::" << time_delay_outpost << std::endl;
      double pitch_offset = result;
      if (msg->v_yaw > OUTPOST_V_THRES) {
        OUTPOST_V_YAW = M_PI * 0.8;
      } else if (msg->v_yaw < -OUTPOST_V_THRES) {
        OUTPOST_V_YAW = -M_PI * 0.8;
      } else {
        OUTPOST_V_YAW = msg->v_yaw;
      }
      //OUTPOST_V_YAW = msg->v_yaw;
      int idx = 0;
      double pre_armor_yaw[4] = {0};
      for (size_t i = 0; i < a_n; i++) {
        pre_armor_yaw[i] = time_delay_outpost * OUTPOST_V_YAW + armor_yaw[i];
        if (pre_armor_yaw[i] > 4 * M_PI) {
          pre_armor_yaw[i] -= 4 * M_PI;
        }
        if (pre_armor_yaw[i] > 2 * M_PI) {
      ...
      // tar_position[i].x = xc - r * cos(tmp_yaw);
      //   tar_position[i].y = yc - r * sin(tmp_yaw);
      p_w.x = xc - r * cos(pre_armor_yaw[idx]) + time_delay_outpost * vx;
      p_w.y = yc - r * sin(pre_armor_yaw[idx]) + time_delay_outpost * vy;
      p_w.z = tar_position[idx].z;
      //Eigen::Vector3d xyz = {p_w.x, p_w.z, p_w.y};
      //////////////计算定点击打需要瞄准的装甲板////////////
      //装甲板的长边0.135m
      double a = small_length;
      double x1 = xa + a * sin(OUTPOST_YAW);
      double y1 = ya - a * cos(OUTPOST_YAW);
      double x2 = xa - a * sin(OUTPOST_YAW);
      double y2 = ya + a * cos(OUTPOST_YAW);
      //double a_distance = (sqrt(x1 * x1 + y1 * y1 + za * za) + (x2 * x2 + y2 * y2 + za * za)) / 2;
      double yaw1 = atan2(y1, x1) / M_PI * 180 + yaw_fix_;
      double yaw2 = atan2(y2, x2) / M_PI * 180 + yaw_fix_;
      // if (yaw1 > 360)
      ...
      //////////////////////////////定点击打逻辑/////////////////////////////
      if (fabs(msg->v_yaw) > OUTPOST_V_THRES) {
        packet.yaw_angle = atan2(ya, xa) / M_PI * 180 + yaw_fix_;
        if (packet.yaw_angle > 360)
          packet.yaw_angle -= 360;
        else if (packet.yaw_angle < 0)
        ...
        packet.pitch_angle =
          -atan2(za, sqrt(xa * xa + ya * ya)) / M_PI * 180 + 90 - pitch_offset + pitch_fix_;
        packet.distance = sqrt(xa * xa + ya * ya + za * za);
      }
      //////////////////////////////跟踪击打逻辑/////////////////////////////
      else {
        packet.yaw_angle = atan2(p_w.y, p_w.x) / M_PI * 180 + yaw_fix_;
      ...
      if (packet.detect_flag != 0) {
        if (
          yaw_error < (atan((0.17/2) / packet.distance)) / M_PI * 180 &&
          pitch_error < (atan((0.12/2) / packet.distance)) / M_PI * 180) {
          if (fabs(msg->v_yaw) > OUTPOST_V_THRES) {
            if ((pre_yaw >= yaw1 && pre_yaw <= yaw2)) packet.shoot_flag = 1;
          } else
            packet.shoot_flag = 1;
        }
    ...
    if (marker_pub_->get_subscription_count() > 0) {
      ...
      // if(a_n == 3)
      // {
      //   aiming_point_.pose.position.x =  p_w_center.x; 
      //   aiming_point_.pose.position.y =  p_w_center.y;
...
void RMSerialDriver::getParams()
{
  ...
  try {
    outpost_time_fix_ = declare_parameter<double>("outpost_time_fix", 0);
  } catch (rclcpp::ParameterTypeException & ex) {
    RCLCPP_ERROR(get_logger(), "The device name provided was invalid");
    throw ex;
  }
...
double RMSerialDriver::dynamicCalcPitchOffset(Eigen::Vector3d & xyz)
{
  ...
  //double bullet_speed = 15.5;            //TODO:弹速可变
